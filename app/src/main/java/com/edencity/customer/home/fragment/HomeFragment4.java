...
import com.edencity.customer.home.adapter.RlvAdapter;
import com.edencity.customer.home.adapter.RlvImaAdapter;
...
public class HomeFragment4 extends BaseFragment2 {
    ...
    private RecyclerView rlv_rlv;
    private RelativeLayout layout_rlv;
    private RelativeLayout layout_img;
    private RecyclerView rlv_img;
    private RlvAdapter chanelAdapter;
    private RlvImaAdapter rlvImaAdapter;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View inflate = inflater.inflate(R.layout.fragment_new_home4, container, false);
    ...
    private void initimg() {
        ...
        DataService.getHomeService().customerEvent(hashMap)
                .subscribeOn(Schedulers.io())
                .unsubscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<BaseResult<CustomerEvent>>() {
                    @Override
                    ...
                    @Override
                    public void onNext(BaseResult<CustomerEvent> baseResult) {


                        if (baseResult.getResult_code() == 0) {

                            Log.e("cccc",baseResult.toString());
                            if (baseResult.getData()!=null && baseResult.getData().getCustomerEvents()!=null
                            && baseResult.getData().getCustomerEvents().size()>0){
                                if (baseResult.getData().getCustomerEvents().size()>1){
                                    rlv_img.setLayoutManager(new GridLayoutManager(getContext(),2));
                                }else{
                                    rlv_img.setLayoutManager(new LinearLayoutManager(getContext()));
                                }
                                layout_img.setVisibility(View.VISIBLE);
                                 rlvImaAdapter = new RlvImaAdapter();
                                rlv_img.setAdapter(rlvImaAdapter);
                                rlvImaAdapter.addData(baseResult.getData().getCustomerEvents());
                                rlv_img.setNestedScrollingEnabled(false);
                                rlv_img.setHasFixedSize(true);
                                rlvImaAdapter.onClick(new RlvImaAdapter.onItemClickListener() {
                                    @Override
                                    public void onClick(int position) {
                                        Intent intent = new Intent(getContext(), NormalWebActivity.class);
                                        intent.putExtra("url",baseResult.getData().getCustomerEvents().get(position).getEventUrl());
    ...
    private void initRlv() {
        ...
        objects.add(new BaseDebug(R.mipmap.food,"美食","1"));
        objects.add(new BaseDebug(R.mipmap.maker,"创客空间","1"));
        objects.add(new BaseDebug(R.mipmap.home,"人才公寓","1"));
        objects.add(new BaseDebug(R.mipmap.funeng,"赋能服务","1"));
        objects.add(new BaseDebug(R.mipmap.youxue,"游学服务","1"));
        objects.add(new BaseDebug(R.mipmap.peixun,"创业培训","1"));
        objects.add(new BaseDebug(R.mipmap.zhishi,"知识产权\n交易服务","1"));
        objects.add(new BaseDebug(R.mipmap.yidian,"2","2"));
        chanelAdapter = new RlvAdapter();
        rlv_rlv.setLayoutManager(new GridLayoutManager(getContext(),4));
        rlv_rlv.setAdapter(chanelAdapter);
        rlv_rlv.setHasFixedSize(true);
        rlv_rlv.setNestedScrollingEnabled(false);
        chanelAdapter.addData(objects);
        chanelAdapter.onClick(position -> {
            if (position==7){
            }else{
                Intent intent = new Intent(getContext(),NormalWebActivity.class);
                int i = position + 1;
                intent.putExtra("url","http://h5.edencitybrand.com/scIndustry.html?&scIndustryId="+i);
                startActivity(intent);
            }
                    ...
                    public void onNext(BaseResult<TeamEntity> baseResult) {
                        ...
                        if (baseResult.getResult_code() == 0) {
                            if (baseResult.getData()!=null && baseResult.getData().getEntryTeams()!=null){
                                ...
                                rlv_team.setLayoutManager(new GridLayoutManager(getActivity(),2));
                                TeamItemAdapter adapter = new TeamItemAdapter(1);
                                rlv_team.setAdapter(adapter);
                                adapter.addData(entryTeams);
                                rlv_team.setHasFixedSize(true);
                                rlv_team.setNestedScrollingEnabled(false);
                                adapter.onClick(position -> {
    ...
    private void initView(View inflate) {
        ...
        rlv_rlv = inflate.findViewById(R.id.rlv_rlv);
        layout_rlv = inflate.findViewById(R.id.layout_rlv);
        rlv_img = inflate.findViewById(R.id.rlv_img);
        layout_img = inflate.findViewById(R.id.layout_img);
